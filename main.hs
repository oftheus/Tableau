...
main = do
  ...
  input <- readFile "inputs/input2.txt"
...
data Token
  = Variavel Char -- Variáveis proposicionais, representadas por caracteres
...
converterInputToken input =
  if null input
    then
      ...
      if head input == '∧'
        then
          E_Token : converterInputToken (tail input)
        else
          if head input == '∨'
            then
              Ou_Token : converterInputToken (tail input)
            else
              if head input == '¬'
                then
                  Nao_Token : converterInputToken (tail input)
                else
                  if head input == '→'
                    then
                      Implicacao_Token : converterInputToken (tail input)
                    else
                      if head input == '↔'
                        then
                          BiImplicacao_Token : converterInputToken (tail input)
                        else
                          if head input == '('
                            then
                              AbrirParenteses_Token : converterInputToken (tail input)
                            else
                              if head input == ')'
                                then
                                  FecharParenteses_Token : converterInputToken (tail input)
                                else
                                  if isAlpha (head input)
                                    then
                                      Variavel (head input) : converterInputToken (tail input)
                                    else
                                      error ("Caractere inválido: " ++ [head input])
...
parseBiImplicacao tokens =
  let (esquerda, rest) = parseImplicacao tokens
   in case rest of
        (BiImplicacao_Token : rest') ->
          let (direita, rest'') = parseBiImplicacao rest'
...
parseImplicacao tokens =
  let (esquerda, rest) = parseOu tokens
   in case rest of
        (Implicacao_Token : rest') ->
          let (direita, rest'') = parseImplicacao rest'
...
parseE tokens =
  let (esquerda, rest) = parseNao tokens
   in case rest of
        (E_Token : rest') ->
          let (direita, rest'') = parseE rest'
           in (E esquerda direita, rest'')
        _ -> (esquerda, rest)

parseNao :: [Token] -> (Formula, [Token])
parseNao (Nao_Token : rest) =
  let (subFormula, rest') = parseAtomo rest
